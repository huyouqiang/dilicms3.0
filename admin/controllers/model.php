...
class Model extends Admin_Controller
{
	...
	public function _add_post()
	{
		if ($this->_validate_model_form() == TRUE)
		{
			...
			$data['belong_group'] = $this->input->post('belong_group', TRUE);
		...
		else
		{
			...
			$data['groupList'] = $this->db->query("select id as class_id,description as class_name,0 as deep from ".$this->db->dbprefix('group')." order by level asc")->result_array();
	...
	public function _edit_post($id = 0)
	{
		...
		if ($this->_validate_model_form($target_model->name) == TRUE)
		{
			...
			$data['hasattach'] = $this->input->post('hasattach', TRUE);
			$data['belong_group'] = $this->input->post('belong_group', TRUE);
			$data['model_sort'] = $this->input->post('model_sort', TRUE);
            //处理缩略图设置
            create_thumb_preferences($data);
