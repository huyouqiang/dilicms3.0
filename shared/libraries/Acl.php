...
class Acl
{
	...
	public function show_left_menus()
	{
		foreach ($this->left_menus as $key => $v)
		{
			...
			{
				echo '<li class="mb-1">
				<button class="btn btn-sm btn-toggle align-items-center rounded" style="font-size:12px;">' . $v['menu_name'] . '</button>
	...
	{//1
		...
		// $this->load->database();
		// die(print_r($this->_current_menu));
		foreach ($this->left_menus as $vkey => & $v)
		{
			foreach ($v['sub_menus'] as $jkey => & $j)
			{
