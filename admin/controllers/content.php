...
class Content extends Admin_Controller
{
     ...
     * @return  array
     */
	private function _pagination($model,$id)
	{
...
//		$condition = array('id'=>);
	...
	public function menuLeft()
	{
		...
		$group =$this->db->select()
						 ->order_by("level","asc")
						 ->get($this->db->dbprefix('group'))
						 ->result_array();

		if(is_array($group)){
			foreach ($group as $key => $value) {
				$menuHtml.='<li class="mb-1">
				<button class="btn btn-toggle align-items-center rounded" style="font-size: 14px;">' . $value['description'] . '</button>
