...
class Model_mdl extends CI_Model
{
	...
	{
		$modelGroup=$this->db->dbprefix('group');
		$model=$this->db->dbprefix('models');
		$modelList=array();

		$modelGroupList=$this->db->select()->from("{$modelGroup}")->order_by("level asc")->get()->result_array();
		if(is_array($modelGroupList)){
			foreach ($modelGroupList as $key => $value) {
				$modelArr=$this->db->select("m.*,(select g.description from {$modelGroup} g where g.id='{$value['id']}') as modelGroupName")->from("{$model} m")->where("m.belong_group",$value['id'])->order_by("m.model_sort asc")->get()->result();
				$modelList=array_merge($modelList,$modelArr);
			}
		}
