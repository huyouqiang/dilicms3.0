...
class Form 
{
	...
	public function display( & $field, $default = '', $has_tip = TRUE, $allow_upload = FALSE)
	{
		...
		if ($has_tip)
		{	
			$filedType=$this->$type($field, $default, $allow_upload);
			echo  $this->_add_tip($field['ruledescription'],$filedType );
		}
		else
		{
     ...
     * @param   string
     * @param   string
     * @return  void . ($default == $v['class_id'] ? 'selected="selected"' : '')
     */
	...
	public function show_class_edit( & $category, $name, $default )
	{
	    ...
	    foreach ($category as $v)
		{
			if($v['class_id']==$default){
				$html .= 	'<option value="' . $v['class_id'] . '" ' . 'selected="selected">';
			}else{
				$html .= 	'<option value="' . $v['class_id'] . '" ' . '>';
			} 
