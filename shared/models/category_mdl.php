...
class Category_mdl extends CI_Model
{
	...
	*/
	...
	public function get_category_models()
	{
		return $this->db->order_by("level asc")->get($this->db->dbprefix('group'))->result();	
	}
	...
	*/
	...
	public function get_category_model_by_id($id)
	{
		return $this->db->where('id', $id)->get($this->db->dbprefix('group'))->row();	
	}
	...
	{
		if ($this->db->insert($this->db->dbprefix('group'), $data))
		{
			return TRUE;
		}
		return FALSE;	
	...
	{
		if ($this->db->where('id', $target_model->id)->update($this->db->dbprefix('group'), $data))
		{
			$this->load->dbforge();
	...
	public function del_category_model($model)
	{
		...
		//删除字段
		$this->db->where('id',$model->id)->delete($this->db->dbprefix('group'));
		// //删除附件
