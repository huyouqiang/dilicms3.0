...
class field_file
{
    ...
    public function on_form($field, $default= '', $has_tip = TRUE)
    {
        ...
        if ($default)
        {
            if (strpos($default, '/') !== 0) $default = '/' . $default;
...
//                 $target_path = DILICMS_SHARE_PATH.'../'.setting('attachment_dir').'/'.$upload['folder'];
                ...
                {
                    $upload['image'] = (in_array($upload['type'], array('jpg', 'gif', 'png', 'jpeg', 'bmp','ipa','apk','mp4','mp3')) ? 1 : 0);
                    //可自行根据需要进行缩略图处理
                    ...
                    if (!!$aid = $CI->db->insert_id())
                    {
                        ...
                        //表单赋值
                        $att =  setting('attachment_dir').'/'.$upload['folder'].'/'.$upload['name'].'.'.$upload['type'];
                        $post[$field['name']] = $att;
