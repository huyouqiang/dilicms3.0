        ...
        <?php echo form_open('model/edit/'.$model->id); ?>
			<table class="form_table dili_tabs" id="site_basic" >
                ...
                <tr>
                    <th> 所属分类模型：</th>
                    <td><?php
                            $this->form->show_class_edit($groupList,"belong_group",$model->belong_group); 
                        ?>
                        <label>*每个内容模型必须选择一个分类。</label><?php echo form_error('perpage'); ?>
                    </td>
                </tr>
                <tr>
                    <th> 排序：</th>
                    <td><?php $this->form->show('model_sort','input','',$model->model_sort); ?><label>*每个内容模型显示顺序。</label><?php echo form_error('perpage'); ?></td>
                </tr>
                <tr>
					<th> 是否使用上传控件：</th>
					<td>
